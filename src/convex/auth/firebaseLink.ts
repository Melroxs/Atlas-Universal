/**
 * Firebase → Convex account linking (V8, typed database access).
 *
 ...
 * internal mutation, invoked via ctx.runMutation from firebaseProvider.ts.
 *
 * Internal (not public): only the auth sign-in flow may call this. Public
 * clients can never fabricate accounts — the Firebase identity passed in is
 * always verified by the Admin SDK action before this mutation runs.
 */
...
export const firebaseEnsureUser = internalMutation({
  args: {
    uid: v.string(),
  ...
  handler: async (ctx, { uid, email, emailVerified, name, picture }) => {
    ...
    // Existing Firebase account -> sign the matching Convex user in,
    // refreshing profile fields when Firebase reports newer values.
    const existing = await ctx.db
      .query("authAccounts")
      .withIndex("providerAndAccountId", (q) =>
        q.eq("provider", "firebase").eq("providerAccountId", uid),
      )
      .first();
    ...
    if (existing) {
      ...
      if (!user) {
        throw new Error("Firebase account is not linked to a valid user.");
      }
    ...
    // First time this Firebase identity is seen -> create the Convex user.
    // Email linking is only allowed when Firebase has actually verified the
    // address, so an unverified account can never claim an existing user's
