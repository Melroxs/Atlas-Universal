// ---------------------------------------------------------------------------
// Atlas Contradiction Engine (canonical, pure).
//
// DEPLOYMENT CONTRACT: this file lives INSIDE the conversation-converse
// function package directory (same rule as gemini.ts / evidence-requirements.ts):
// no Deno imports, no repository imports — the edge function, the browser
// fallback and the vitest suite all import it directly.
//
// Responsibilities (deterministic, evidence-grounded):
//   - extract labeled values (amounts, roof area SQ, loss dates, deductibles)
//     from document text
//   - group values by claim number found in the documents
//   - report any field that carries two or more DISTINCT values for the same
//     claim, preserving BOTH sources (never silently picking one)
//   - cross-field checks (estimate vs invoice, roofing vs carrier payment)
//   - compare a persisted claim record against its documents (claim-vs-doc)
//
// Every contradiction carries both values, their source documents, and a
// deterministic severity. A difference is flagged for human reconciliation —
// it is NOT proof of an error (supplements, allowances and adjustments are
// legitimate causes).
// ---------------------------------------------------------------------------

import type { GapSeverity } from "./evidence-requirements.ts";
...
const LABEL_PATTERNS: Array<{ field: string; kind: EvidenceContradiction["kind"]; re: RegExp; amount?: boolean }> = [
  {
...
function severityFor(field: string, kind: EvidenceContradiction["kind"], a: number | null, b: number | null): GapSeverity {
...
function detailFor(field: string, kind: EvidenceContradiction["kind"], values: ContradictionValue[]): string {
  const listed = values
...
export function scanDocumentsForContradictions(
  ...
  for (const d of relevant) {
    ...
    const claim = claimNumberIn(text) ?? claimNumberIn(`${d.title ?? ""} ${d.classification ?? ""}`);
    for (const { field, kind, re, amount } of LABEL_PATTERNS) {
      ...
      for (const m of matches) {
        const raw = m[1] ?? m[0];
        // Amounts are normalized to $N; quantities keep the full match (e.g.
        // "28.7 SQ") so the unit is never lost; dates keep the full match.
        const value = kind === "amount" ? `$${raw}` : kind === "quantity" ? m[0] : m[0] || raw;
        if (seen.has(value)) continue;
        seen.add(value);
        push(field, { claim, value, doc: d, amount: amount ? num(raw) ?? undefined : undefined });
      }
    }
  }
      ...
      for (const cb of pb) {
        if (ca.claim === cb.claim && ca.value !== cb.value && ca.claim) {
          const values: ContradictionValue[] = [
            { value: ca.value, documentId: ca.doc._id, documentTitle: ca.doc.title ?? "Document" },
            { value: cb.value, documentId: cb.doc._id, documentTitle: cb.doc.title ?? "Document" },
          ];
...
export function compareClaimAgainstDocuments(
  ...
  const textOf = (docs: ContradictionDoc[]) => docs.map((d) => d.text ?? "").join("\n");
